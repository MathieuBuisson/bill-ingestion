...
def test_config_success(mock_is_file, mock_mkdir, mock_getenv, mock_load_dotenv, mock_env):
...
@patch('pathlib.Path.mkdir')
@patch('pathlib.Path.is_file', return_value=True)
def test_config_missing_required_var(mock_is_file, mock_mkdir, mock_getenv, mock_load_dotenv, mock_env):
    """Test that Config raises ConfigurationError when a required variable is missing."""
    mock_env["BORDGAIS_EMAIL"] = ""
    mock_getenv.side_effect = lambda k, d=None: mock_env.get(k, d)
    
    with pytest.raises(ConfigurationError, match="Missing required environment variables: BORDGAIS_EMAIL"):
        Config()
...
def test_config_missing_credentials_file(mock_is_file, mock_mkdir, mock_getenv, mock_load_dotenv, mock_env):
