...
def test_converter_init_missing_folder():
    ...
    with pytest.raises(ConversionError, match="MARKDOWN_DESTINATION_FOLDER configuration is not set"):
...
def test_convert_success(mock_open_file, mock_unlink, mock_exists, mock_mkdir, mock_tempfile, mock_to_markdown, mock_config):
...
def test_convert_pymupdf_error(mock_tempfile, mock_to_markdown, mock_config):
    ...
    converter = PDFToMarkdownConverter(mock_config)
    with pytest.raises(ConversionError, match="Failed to convert PDF 'test.pdf' to Markdown"):
        converter.convert("test.pdf", b"some data")
